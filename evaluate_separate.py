...
class CLIPScore:
    ...
    def __call__(self, predictions, images_root):
        ...
        for img_name, pred_caption in predictions.items():
            ...
            CLIPScore = self.getCLIPScore(image, pred_caption[:])
            self.output[img_name] = CLIPScore
...
def main(args):
    ...
    with open("P2_CLIPScore.json", "w") as f:
        # sort CLIPScore().output by value
        CLIPScore().output = dict(sorted(CLIPScore().output.items(), key=lambda x: x[1], reverse=True))
        json.dump(CLIPScore().output, f, indent=4)
